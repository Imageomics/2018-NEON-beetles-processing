...
def crop_beetles(img, mask, padding=50, cropped_dim=(256, 256)):
  '''Goes through the predicted mask of an image and crops down the original image to each of the available beetles
  based on the segmention mask'''

  cropped_wings = dict()

  #only search for masks belonging to right/left hindwings and forewings
  for wing_class in [1]: #[2,3,4,5]:
    # img = test_img #[:,:, 0]
    mask = np.asarray(mask==wing_class, dtype=int) #0s and 1s
...
def main():
    ...
    for (image, mask), fp in zip(zip(dataset_images, dataset_masks), image_filepaths):   
        ...
        cropped_wings, cropped_wings_resized = crop_beetles(image, mask, args.pad)
